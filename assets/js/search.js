...
(function () {
  ...
  function fuzzyMatch(text, query) {
    ...
    if (query.length >= 3) {
      for (const word of text.split(/\s+/)) {
        ...
        if (dist <= Math.floor(query.length / 4) + 1) return { match: true, score: 80 - dist * 5 };
      }
    }
  ...
  class ChervepediaSearch {
    ...
    loadArticles() {
      ...
      try { this.articles = JSON.parse(dataScript.textContent); } catch {}
    }
    ...
    handleKeydown(e) {
      ...
      else if (e.key === 'Enter' && active) { e.preventDefault(); window.location.href = active.href; return; }
      else if (e.key === 'Escape') { this.hideResults(); return; }
      else { return; }
    ...
    renderResults(results) {
      if (!results.length) { this.results.innerHTML = '<div class="search-no-results">Ничего не найдено</div>'; return this.showResults(); }
    ...
    showResults() { this.results.classList.add('active'); }
    hideResults() { this.results.classList.remove('active'); }
  }

  /* ==========================
     RANDOM ARTICLE (WORKS EVERYWHERE)
     ========================== */
  ...
  class RandomArticle {
    ...
    async loadArticles() {
      const dataScript = document.getElementById('search-data');
      if (dataScript) {
        try { this.articles = JSON.parse(dataScript.textContent); return; } catch {}
      }
